...
class Tasks:
    ...
    async def perform_tasks(self):
        if not utils.getConfig("auto_tasks", True):
            ...
            return True
        self.log.info("Performing tasks.")
        ...
        try:
            ...
            for task in incompleted_tasks:
                self.log.info(f"Performing task: <y>{task.name}</y> ...")
                ...
                if task.name == "Like and repost the latest news":
                    if self.finish_task(task):
                        self.log_task_reward(task)
                elif task.task_type == "normal" and task.type == "web_app_open_link":
                    if self.finish_task(task):
                        self.log_task_reward(task)
                    ...
                    task.class_name == "Partner Task"
                    and task.type == "open_link"
                    and task.task_type == "normal"
                ):
                    ...
                    if task.copy_text not in self.profile.user_profile.nickname:
                        ...
                        if self.finish_task(task):
                            self.log_task_reward(task)
                elif "t.me" in task.url and utils.getConfig("auto_join_channels", True):
                    if not self.tgAccount:
                        continue
